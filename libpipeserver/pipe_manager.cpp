...
void PipeManager::runServer()
{
    while (m_running)
    {
        ...
        {
            std::cout << "Failed to Connect(PIPE SERVER)" << std::endl;
        }
...
void PipeManager::handleClient(HANDLE hPipe)
{
    ...
    while (m_running)
    {
            ...
            nullptr); !success || bytesRead == 0)
        {
            ...
            {
                std::cout << "Client disconnected succesfully." << std::endl;
            }
            else
            {
                std::cout << "Failed to Read from Pipe Server: " << error << std::endl;
            }
            break;
        }
        ...
        std::cout << "Received Messages: " << message.c_str() << "\n" << bytesRead << std::endl;
